...
class PairwiseMeasures(object):
    def __init__(self, ref_img, seg_img, analysis='microbleeds',
                 measures=None, connectivity=3, pixdim=[1, 1, 1],
                 empty=False, list_labels=None, threshold=0.5, thresh_assign=3):
        ...
        self.m_dict = {
            ...
            'tp': (self.tp_count_element, 'TP num'),
            'fp': (self.fp_count_element, 'FP num'),
            'fn': (self.fn_count_element, 'FN num')
        }
    ...
    def mean_dsc_perimage(self):
        ...
        for r in unique_ref:
            ...
            for s in list_seg:
                temp_seg += np.where(label_seg == s, np.ones_like(label_seg), np.zeros_like(label_seg))
            dsc_temp = (2. * np.sum(temp_seg * temp_ref)) / np.clip(np.sum(temp_seg) + np.sum(temp_ref), a_min=1e-5, a_max=1e10)
    ...
    def f1_count_element(self):
        ...
        return (2 * numb_tp) / np.clip(numb_fn + numb_fp + 2 * numb_tp, a_min=1e-5, a_max=1e10)

    def tp_count_element(self):
        df_classif = self.df_classif
        return df_classif[df_classif['category'] == 'TP'].shape[0]

    def fp_count_element(self):
        df_classif = self.df_classif
        return df_classif[df_classif['category'] == 'FP'].shape[0]

    def fn_count_element(self):
        label_ref = self.ref_cc
        df_classif = self.df_classif
        numb_tp = df_classif[df_classif['category'] == 'TP'].shape[0]
        return np.max(label_ref) - numb_tp
